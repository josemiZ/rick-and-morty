...
fun CharacterListScreen(
    ...
    LaunchedEffect(shouldLoadMore) {
        if (shouldLoadMore) onAction(CharacterListAction.LoadMore)
    }
    LaunchedEffect(uiState.error) {
        if (uiState.error != null && uiState.characters.isNotEmpty()) {
    ...
    Scaffold(
        ...
        PullToRefreshBox(
            ...
            onRefresh = { onAction(CharacterListAction.Refresh) },
                ...
                uiState.characters.isEmpty() ->
                    ...
                    items(items = uiState.characters, key = { it.id }) { character ->
                        CharacterCard(
                            ...
                            onFavoriteClick = {
                                onAction(CharacterListAction.ToggleFavorite(character.id, !character.isFavorite))
                            }
                        )
                    }
...
private fun CharacterListScreenPreview(
    @PreviewParameter(CharacterListPreviewProvider::class) uiState: CharacterListUiState
) {
    RickAndMortyTheme {
        CharacterListScreen(
            uiState = uiState,
